...
def plot_umap(
    ...
    title: str = "UMAP of GNN Features",
    cmap: str = "viridis",
    pca_dim: int = 50
):
    """
    ...
    参数说明:
        n_neighbors: 控制局部/全局结构权衡。值小关注局部（类似小 perplexity），
                     值大关注全局结构。默认 15。
        min_dist:    控制点在低维空间的最小间距。默认 0.1，增大则簇更松散。
    """
    try:
        import umap
    except ImportError:
        raise ImportError(
            "UMAP not installed. Please install it first:  pip install umap-learn"
        )

    # 1. 可选 PCA 预处理（UMAP 对高维也鲁棒，但 PCA 到 50D 可进一步加速）
    if features.shape[1] > pca_dim:
        print(f"PCA preprocessing: {features.shape[1]}D -> {pca_dim}D")
        features = PCA(n_components=pca_dim, random_state=42).fit_transform(features)
